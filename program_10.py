...
Lab10 - Descriptive Statistics and Environmental Metrics
...
def CalcTqmean(Qvalues):
    ...
    #Remove the no value data
    Qvalues = Qvalues.dropna()
    #Calculate Tqmean
    Qmean = Qvalues.mean()
    Qlarge = (Qvalues > Qmean).sum()    
    Tqmean = Qlarge/(len(Qvalues))
...
def CalcRBindex(Qvalues):
    ...
    #Remove the no value data
    Qvalues = Qvalues.dropna()
    #Calculate the sum of the absolute values of changes in daily discharge
    diff = Qvalues.diff()
    abs_diff = diff.abs()
    sum_change = abs_diff.sum()
    #Calculate total discharge
    total_dis = Qvalues.sum()
    #Calcualte RBindex
    RBindex = sum_change / total_dis
...
def Calc7Q(Qvalues):
    ...
    #Remove the no value data
    Qvalues = Qvalues.dropna()
    #Calcualte the lowest 7-day flow average
...
def CalcExceed3TimesMedian(Qvalues):
    ...
    #Remove the no value data
    Qvalues = Qvalues.dropna()
    #Count the events that are 3 times greater than median value of data
...
def GetAnnualStatistics(DataDF):
    ...
    #Resample water year from october
    WYDataDF = DataDF.resample('AS-OCT').mean()
    #Create new dataframe
    WYDataDF = pd.DataFrame(0,index = WYDataDF.index, columns = colname)
    
    #Add in the annual values of each column
    ...
    #Using function completed above
...
def GetMonthlyStatistics(DataDF):
    ...
    #Resample monthly statistics
    MSDataDF = DataDF.resample('MS').mean()
    #Create new dataframe
    MoDataDF=pd.DataFrame(0,index=MSDataDF.index,columns = colname)
    
    #Add in the monthly stat values of each column
...
if __name__ == '__main__':
    ...
    #Output csv file of Annual Metrics
    #Wildcat
    WCWY = WYDataDF['Wildcat']
    WCWY['Station'] = 'Wildcat'
    #Tippe
    TPWY = WYDataDF['Tippe']
    TPWY['Station'] = 'Tippe'
    #Combine
    WCWY = WCWY.append(TPWY)
        
    WCWY.to_csv('Annual_Metrics.csv', sep =' ', index =True)
        
    #Output csv file of Monthly Metrics
    #Wildcat
    WCMO = MoDataDF['Wildcat']
    WCMO['Station'] = 'Wildcat'
    #Tippe
    TPMO = MoDataDF['Tippe']
    TPMO['Station'] = 'Tippe'
    #Combine
    WCMO = WCMO.append(TPMO)
        
    WCMO.to_csv('Monthly_Metrics.csv', sep =' ', index =True)
        
    #Output txt file of Average Annual Metrics
    #Wildcat
    WCAA = AnnualAverages['Wildcat']
    WCAA['Station'] = 'Wildcat'
    #Tippe
    TPAA = AnnualAverages['Tippe']
    TPAA['Station'] = 'Tippe'
    #Combine
    WCAA = WCAA.append(TPAA)
        
    WCAA.to_csv('Average_Annual_Metrics.txt', sep ='\t', index =True)
        
    #Output txt file of Average Monthly Metrics
    #Wildcat
    WCAM = MonthlyAverages['Wildcat']
    WCAM['Station'] = 'Wildcat'
    #Tippe
    TPAM = MonthlyAverages['Tippe']
    TPAM['Station'] = 'Tippe'
    #Combine
    WCAM = WCAM.append(TPAM)
    
    WCAM.to_csv('Average_Monthly_Metrics.txt', sep ='\t', index =True)
